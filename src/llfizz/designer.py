...
from llfizz.constants import AMINOACIDS, N_AA, CHARGE, SCORE_DB_DIR
...
class FeatureDesigner:
    ...
    def design_loop(
        ...
        """
        query_fvec, _ = Featurizer(self.featurizer_func_dict).featurize(
            "query_sequence", query_sequence, feature_set
        )
        ...
        if scd in self.featurizer_func_dict.values():
            if scd_machine is None:
                previous_scd = query_fvec.features[
                    query_fvec.features["name"] == "scd"
                ]["value"].item()
        ...
        current_distance_to_target += self.convergence_threshold + 1
        ...
        while (
            ...
            current_distance_to_target = next_distance_to_target
            all_candidates = [(query_sequence, copy(scd_machine))]
                ...
                for old_candidate, old_scd_machine in all_candidates:
                    ...
                    try:
                        candidate_fvec, _ = featurizer.featurize(
                            "candidate", candidate, feature_set
                        )
                    except acceptable_errors:
                        continue

                    candidate_distance_to_target = self.metric.euclidean_norm_of(
                        candidate_fvec
                    )

                    if scd_machine is not None:
                        assert scd_key is not None
                        new_scd = candidate_fvec.features[
                            query_fvec.features["name"] == "scd"
                        ]["values"]
        ...
        for mutation_pos, mutation_to in mutation_generator:
            if (
                guess := apply_mutation(
                    current_sequence, mutation_pos, mutation_to, scd_machine
                )
            ) is None:
                continue

            try:
                guess_fvec, _ = featurizer.featurize("guess", guess, feature_set)
            except acceptable_errors:
                continue
            ...
            if guess_distance_to_target < current_distance_to_target:
                if (
                    current_distance_to_target - guess_distance_to_target
                    > self.convergence_threshold
                ):
                    add_mutation_to_memo(
                        mutation_pos,
                        mutation_to,
                        guess_distance_to_target,
                        good_mutation_distances,
                    )
                else:
                    add_mutation_to_memo(
